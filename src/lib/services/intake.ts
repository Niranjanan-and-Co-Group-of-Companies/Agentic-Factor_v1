...
const PROVIDER_KEYWORDS: Array<{ keywords: string[]; provider: string }> = [
  ...
  { keywords: ['canva', 'canva design', 'canva template'], provider: 'canva' },
];
...
- MISSION TITLE NAMING: Never include internal tool or infrastructure names ("Composio", "E2B", "Supabase", "Inngest") in the mission title or description. Use only business-facing language the customer would recognize. Good: "Send Gmail Email", "Post to Facebook". Bad: "Send Gmail Email via Composio", "Run E2B Script".
...
- CANVA: Use composio_oauth type, service "canva". Use composio_execute() for ALL Canva operations. Critical actions (use exact slugs): CANVA_CREATE_CANVA_DESIGN_WITH_OPTIONAL_ASSET (create new design, params: design_type e.g. "presentation", title), CANVA_LIST_USER_DESIGNS (list existing designs), CANVA_CREATE_CANVA_DESIGN_EXPORT_JOB (start export, params: designId, format "PNG"/"PDF"/"MP4"), CANVA_GET_DESIGN_EXPORT_JOB_RESULT (poll export until status="success", returns urls[]), CANVA_INITIATE_CANVA_DESIGN_AUTOFILL_JOB (fill brand template with data, params: brandTemplateId, data dict), CANVA_RETRIEVE_DESIGN_AUTOFILL_JOB_STATUS (poll autofill job), CANVA_CREATE_ASSET_UPLOAD_JOB (upload image by URL), CANVA_RETRIEVE_BRAND_TEMPLATE_DATASET_DEFINITION (get template field schema). Export pattern: 1) create export job → get jobId, 2) poll CANVA_GET_DESIGN_EXPORT_JOB_RESULT with jobId until status="success", 3) use urls[] from result. NEVER make raw Canva API calls — always composio_execute().
