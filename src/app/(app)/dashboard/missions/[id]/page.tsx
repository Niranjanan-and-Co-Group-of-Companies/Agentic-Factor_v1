...
const RenderNode = ({ data }: { data: any }): any => {
  if (data === null) return <span style={{ color: "var(--text-muted)" }}>null</span>;
  if (typeof data !== 'object') {
    if (typeof data === 'string' && data.startsWith('http')) {
      return <a href={data} target="_blank" rel="noreferrer" style={{ color: "var(--accent)", textDecoration: "underline" }}>{data}</a>;
    }
  ...
  if (Array.isArray(data)) {
    return (
      <div style={{ paddingLeft: "var(--space-md)", borderLeft: "2px solid var(--border)", display: "flex", flexDirection: "column", gap: "var(--space-sm)", marginTop: "var(--space-xs)" }}>
        {data.map((item, i) => (
          <div key={i} style={{ background: "var(--bg-glass)", padding: "var(--space-sm)", borderRadius: "var(--radius-sm)" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-xs)" }}>
      {Object.entries(data).map(([key, value]) => {
        ...
        return (
          <div key={key} style={{ 
            background: isLongString || isComplex ? "transparent" : "var(--bg-card)", 
            padding: isLongString || isComplex ? "0" : "var(--space-xs) var(--space-md)", 
            borderRadius: "var(--radius-sm)",
            marginBottom: isLongString || isComplex ? "var(--space-sm)" : "0",
            display: isLongString || isComplex ? "block" : "inline-flex",
            marginRight: isLongString || isComplex ? "0" : "var(--space-sm)",
            alignItems: "center"
          }}>
            <strong style={{ 
              color: isLongString || isComplex ? "var(--accent)" : "var(--text-muted)", 
              fontSize: "0.8rem", 
              marginRight: "var(--space-sm)", 
              textTransform: "capitalize",
              display: isLongString || isComplex ? "block" : "inline",
              marginBottom: isLongString || isComplex ? "var(--space-xs)" : "0"
            }}>{key.replace(/_/g, ' ')}:</strong>
            
            <div style={{ 
              color: "var(--emerald)", 
              fontSize: isLongString ? "0.9rem" : "0.85rem",
              whiteSpace: isLongString ? "pre-wrap" : "normal",
              lineHeight: isLongString ? 1.6 : 1,
              background: isLongString || isComplex ? "var(--bg-card)" : "transparent",
              padding: isLongString || isComplex ? "var(--space-sm)" : "0",
              borderRadius: "var(--radius-sm)",
              border: isLongString || isComplex ? "1px solid var(--border)" : "none",
            }}>
              <RenderNode data={value} />
            </div>
          </div>
        );
      })}
...
const renderFinalOutput = (outputStr: string) => {
  try {
    const parsed = JSON.parse(outputStr);
    if (typeof parsed !== 'object' || parsed === null) throw new Error("Not an object");
    
    return <RenderNode data={parsed} />;
  } catch (e) {
