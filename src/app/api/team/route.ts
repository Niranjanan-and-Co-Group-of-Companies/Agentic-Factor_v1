...
export async function POST(request: NextRequest) {
  ...
  if (!['admin', 'collaborator', 'editor', 'viewer'].includes(role)) {
    return NextResponse.json({ error: 'Role must be admin, collaborator, or viewer' }, { status: 400 });
  }
  ...
  if (seatLimit === 0) {
    return NextResponse.json(
      { error: `Team invites are not available on the ${plan} plan. Upgrade to Pro to invite up to 25 members.` },
      { status: 403 }
    );
  }
  ...
  if (seatLimit < 999999) {
    ...
    if (currentSeats >= seatLimit) {
      return NextResponse.json(
        { error: `You've used all ${seatLimit} seats on your ${plan} plan. Remove a member or upgrade to Enterprise for unlimited seats.` },
        { status: 403 }
      );
    }
  }
...
export async function PATCH(request: NextRequest) {
  ...
  if (!memberId || !['admin', 'collaborator', 'editor', 'viewer'].includes(role)) {
